...
1. 유저가 중앙 서버에 연결되면, 채널 서버 선택 페이지로 이동합니다.

2. 채널 서버를 선택하면 중앙 서버로부터 해당 서버의 주소와 JWT 토큰을 받아, 선택한 서버로 연결을 시도합니다.

3. 채널 서버와의 연결이 성공하면, 채널 이동 페이지로 전환됩니다.

4. 유저가 채널 서버 선택 페이지로 돌아가면, 현재 접속 중인 채널 서버의 유저 수를 감소시키고, 채널 서버는 중앙 서버에 유저 퇴장 신호를 보내어 중앙 서버의 채널 유저 수를 감소시킵니다.
